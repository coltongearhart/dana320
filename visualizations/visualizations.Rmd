...
# -> code chunks are 100% independent, so have to load data everytime want to use it or put in setup
# -> so repeating any sections where saving result for later

# load basketball data
load("www/data-bsu-game.RData")
...
# skipping from my book
# -> storing plot as g and then adding to (g + ...)
# -> bar graphs when geom_bar() has to aggregate over count data
# -> discussion about geom_density for discrete data
# -> violin plots
...
<!-- NOTE: mention that "R has several systems for making graphs, but ggplot2 is the best and easy because just have to learn one system and can apply it lots of places -->
...
<!-- NOTE: when presenting, switch over to an R script and demonstrate the help pages and also how to load the package. -->

<!-- Mention that recommended best practice for them while following along with the lecture is to try all of the code in their own R script and code along with me and make comments for themselves -->
...
<!-- NOTE: demonstrate the help page and the :: notation with palmerpenguins and caret::train -->
...
<!-- NOTE: demonstrate viewing data sets with the viewer and with printing -->
...
<!-- NOTE: run then add data and run again -->

```{r canvas, exercise = TRUE}
```
...
<!-- NOTE: code without aes and run code to throw error -->

```{r layer, exercise = TRUE}
ggplot(data = diamonds)
```
...
<!-- NOTE: add color = purple, mention same error as before; change to quotes -->
...
- So when it doesn't find one, a new column _purple_ is made and assigned this *same value to every observation*. And because this new column is being mapped to the color of the dots, `ggplot()` colors accordingly (although it's not what we wanted).
...
<!-- NOTE: mention will have local aes statements later when using multiple geoms -->

```{r color-constant, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat,
           y = price,
           color = "purple")) + 
...
- To see how this works, let's take a look at the _iris_ dataset.

<!-- NOTE: show iris dataset help page -->
...
<!-- NOTE: color = Species, size = 10, shape = "*" -->
...
- Using the code chunk below, create a scatterplot using a sample of the `diamonds` dataset that includes the following features (code for the sample is provided, check the help page to see how it works!):
  ...
  c) All observations have the shape of a diamond.

<div id="aes-ex2-hint">
**HINT:** Google "geom_point() shapes".
</div>

```{r aes-ex2, exercise = TRUE}
...
<!-- NOTE: look at help pages, add aes and hist, then run code; then a few other aesthetics (binwidth (0.1), fill (red), color (purple) (mess up first with color vs fill and non quoted constants)) and rerun -->
...
- Now let's use the _iris_ data to make another histogram, except we are going to incorporate another variable; specifically we want to visualize on the petal lengths *based on which species they are*.
...
<!-- NOTE: mention now using fill / color as a variable, not constant; show color first, then fill = Species -->

```{r hist-color-aes, exercise = TRUE}
ggplot(data = iris,
       aes(x = Petal.Length)) + 
  geom_histogram()
```
...
<!-- NOTE: come back to and make base R hist and how much easier it is to customize with ggplot and more professional looking -->
...
<!-- NOTE: do log(price) and price/1000 mention changing labels, and add theme -->

```{r boxplot-cust, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = cut,
           y = price)) + 
...
<!-- NOTE: mention that ggplots can handle almost all situations, you just have to understand the format of the data and how to supply it to ggplot -->
...
<!-- NOTE: mention by default, ggplot does a statistical transformation that gets the number of rows for each group and plots them -->

```{r bargraph-raw, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = cut))
```
...
<!-- NOTE: run table and say looks nice, but ggplot needs dataframes; can check class, so would need to wrap in dataframe; then say tidyverse much better and will see more later -->

```{r freq-table, exercise = TRUE}
...
- Using the code chunk below, create a bar graph of `Species` using a sample from the _iris_ dataset provided below.

<div id="bargraph-ex-hint">
**HINT:**  Make sure to inspect the data first.
</div>

<!-- NOTE: mention did some stuff behind the scenes to make you have to check the dataset before continuing -->
...
```{r bargraph-stacked, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = cut)) + 
  geom_bar()
```
...
```{r bargraph-prop-stacked, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = cut,
           fill = clarity)) + 
  geom_bar()
...
```{r bargraph-prop-side, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = cut,
           fill = clarity)) + 
  geom_bar(position = "fill")
...
```{r bargraph-2vars, exercise = TRUE}
```
...
<!-- NOTE: mention this is plotting all cuts -->

```{r facet-wrap, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = table,
           y = depth)) + 
  geom_point()
...
<!-- NOTE: run code first (hard to read cause small panels, and have to rely on the scale numbers), then add ncol = 1 (colums cause want to show difference in X-->

```{r facet-wrap-1col, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat)) + 
  geom_boxplot() + 
  facet_wrap(~ cut)
...
<!-- NOTE: switch to ncol and see worse -->

```{r facet-wrap-bad, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = price)) + 
  geom_histogram() + 
  facet_wrap(~ clarity,
...
<!-- NOTE: mention "free_y" accomplishes what we want here cause the numeric variable is on the y-axis -->

```{r facet-scales, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = price)) + 
  geom_histogram() + 
  facet_wrap(~ cut)
...
<!-- NOTE: run code for an overall boxplot first, then add facet_grid(), mention this is overplotting, easy to do with many levels, demonstrate switching rows and cols too; even fixing scales wont help -->

```{r facet-grid, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = table)) + 
  geom_boxplot()
```
...
<!-- NOTE: mention did some stuff behind the scenes to make less levels so plots are smaller -->
...
<!-- NOTE: run to see where have been, then take off scales and add y =a fter_stat(density); can mention that ggplot takes raw data, geom_histogram does a transformation and after stat accesses that and uses that as y; point out the y axis before and after -->

```{r density-hists, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat)) + 
  geom_histogram() + 
  facet_wrap(~ cut,
...
<!-- NOTE: change number of bins a few times -->

```{r density-hist_bins, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat,
           y = after_stat(density))) + 
...
- Using the code chunk below, use the _iris_ dataset to do the following:
  ...
  b. Overlay a density curve of `Petal.Length`.
  
<div id="density-curve-ex-hint">
**HINT:** You can plot multiple geoms on the same plot simply by adding another layer.
</div>

```{r density-curve-ex, exercise = TRUE}
```
...
<!-- NOTE: add linetype first, then color, then linewidth -->

```{r density-curves, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat)) + 
  geom_density()
```
...
<!-- NOTE: add parameter and try a few values -->

```{r density-curve_adjust, exercise = TRUE}
ggplot(data = diamonds,
       aes(x = carat)) + 
  geom_density()
```
...
```{r density-curves-ex, exercise = TRUE}
```
...
<!-- NOTE: mention no solution, but will know when get it right -->
