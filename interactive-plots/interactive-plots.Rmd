...
econ <- economics %>%
  mutate(year = year(date), mnnth = month(date))
...
# Add textbook references in Overview
# -> 
...
```{r ggplotly-bargraph, exercise = TRUE}
...
- Here is a demonstration of the types of interactivity that `ggplotly()` gives us for various plot types we already know very easily.

- Side-by-side barplots. With multiple aesthetics being mapped to, there are more interactivity features available after converting to a **plotly** object.

```{r ggplotly-side-by-side-bargraphs, exercise = TRUE}
p <- ggplot(data = diamonds,
            aes(x = cut,
                fill = clarity)) + 
...
```{r ggplotly-histograms, exercise = TRUE}
p <- ggplot(data = diamonds,
            aes(x = price)) + 
  geom_histogram() + 
  facet_grid(cut ~ .,
...
```{r ggplotly-boxplots, exercise = TRUE}
p <- ggplot(data = diamonds,
       aes(x = cut,
           y = price)) + 
  geom_boxplot()
...
```{r ggploty-stacked-bargraph-ex, exercise = TRUE}
```
...
<!-- NOTE: do alpha for all data, then do slice sample for 10,000 points to show works for this number -->

```{r ggplotly-scatterplots, exercise = TRUE}
p <- ggplot(data = diamonds,
       aes(x = log(carat),
           y = log(price))) + 
  geom_point()
...
```{r ggplotly-hexmap, exercise = TRUE}
p <- ggplot(data = diamonds,
            aes(x = log(carat),
                y = log(price))) + 
  geom_point()
...
<!-- NOTE: start with freqpoly and switch to geom_density -->

```{r ggplotly-freqpoly, exercise = TRUE}
```
...
<!-- NOTE: clean scales (comma, then millions) and labels and hard code year in title first, then show data-driven trick, then make interactive and point out hover text and add country, and legend doesn't come in quite right -->

```{r gapminder-bubble-ggplotly, exercise = TRUE}
```
...
- Using `plot_ly()` gives us the interactivity automatically and allows us to give mores customizations to features. We will start with the basics.
...
<!-- NOTE: start without ~ to show error, point out the second one is a heatmap (a new type of chart that colors by count for the intersections) -->
...
```{r plotly-colors-constant, exercise = TRUE}
```
...
```{r plotly-histogram, exercise = TRUE}
```
...
<!-- NOTE: sequentially go through first sequence, mention how group by and summarize aren't changing the plot, but the data underneath for the next layer -->

```{r plotly-bargraph-labels, exercise = TRUE}
```
...
- `add_bars()` and `add_histogram()` work the same way as **ggplot** `geom_bar()` `geom_histogram()`, respectively.

    - The main difference between them is that bar traces require bar heights (both `x` and `y`), whereas histogram traces require just a single variable, and it handles binning automatically (i.e. it performs statistics dynamically in the web browser).
...
<!-- NOTE: show head(mtcars) and the rownames are not a variable, just showing another function that is available  -->
...
<!-- NOTE: point out have to convert to factor from double, or else will get a histogram, then can change to pareto chart by reordering and do -->
    
```{r plotly-bar-vs-hist, exercise = TRUE}
```
...
```{r plotly-bar-vs-hist-ex, exercise = TRUE}
```
...
<!-- NOTE: point out can make horizontal or vertical, start with x = cut and y = price, then switch to horizontal-->
<!-- - point out currently, all outliers are being shown (all points filled in), can change that option via boxpoints = argument, then show schema(), change to suspectedoutliers (empty cicles, hard to see), then none = FALSE -->

```{r plotly-boxplots, exercise = TRUE}
```
...
<!-- NOTE: copy above code, get rid of boxpoints, and add mutate line, add boxmean, right-skewed distributions -->

```{r plotly-boxplots-sorted, exercise = TRUE}
```
...
```{r plotly-boxplots-sorted-ex, exercise = TRUE}
```
...
<!-- NOTE: start with just country in label, then make fancy -->

```{r gapminder-bubble-plotly, exercise = TRUE}
```
...
- Instead of filtering the data down to one year, we can use the whole gapminder dataset and add `frame = ~year` (or `aes(frome = year)`), which will make the visualization into an animation. By default, animated views come with a play/pause button(s) and a slider component for controlling the animation. These can be customized; see [Chapter 14](https://plotly-r.com/animating-views).

<!-- NOTE: copy code, delete filter, and add frame -->

```{r gapminder-bubble-plotly-frame, exercise = TRUE}
```
...
```{r plotly-scatterplot-ex, exercise = TRUE}
```
...
<!-- NOTE: write with paths, then copy code for lines, then arrange by sunspots and rerun each code -->

```{r path-line, exercise = TRUE}
...
```{r multiple-line-plot1, exercise = TRUE}
head(economics)
econ <- economics %>%
...
```{r multiple-line-plot2, exercise = TRUE}
```
...
```{r multiple-line-plot3, exercise = TRUE}
```
...
```{r gapminder-line-plot1, exercise = TRUE}
```
...
<!-- NOTE: add, run and then comment out plotly_data() -->

```{r gapminder-line-plot2, exercise = TRUE}
gapminder %>% 
  group_by(country) %>% 
  plot_ly(x = ~year, y = ~lifeExp) %>% 
  add_lines(color = ~continent)
...
```{r histogram-2d-heatmap-ex, exercise = TRUE}
```
...
<!-- NOTE: comment out layout before running through data steps -->

```{r slope-graph-plotly, exercise = TRUE}
gapminder %>% 
  filter(year %in% c(min(year), max(year))) %>% 
  summarize(.by = c(continent, year),
           avg_lifeExp = round(mean(lifeExp), 1)) %>% 
...
<!-- NOTE: add legend items last -->

```{r dumbell-plot-plotly, exercise = TRUE}
mpg %>% 
  summarize(.by = model,
            across(c(cty, hwy), mean)) %>% 
  mutate(model = fct_reorder(model, cty)) 
...
```{r slopegraph-dumbell-ex, exercise = TRUE}
```

```{r slopegraph-dumbell-ex-ex-solution}
# part a)
...
<!-- NOTE: order = "anyClass" provides an optimal order of the axes according to some criteria that we don't need to know about -->
<!-- - get decent interactivity with ggplotly -->
...
```{r parallel-coord-plotly, exercise = TRUE}
```
...
<!-- NOTE: add highlight_key(), show results, then customize how selected via highlight() -->

```{r basic-graphical-query, exercise = TRUE}
mtcars %>% 
  plot_ly(x = ~wt,
          y = ~mpg) %>% 
  add_markers() %>% 
...
<!-- NOTE: starting with same code as previous, just have to create a shared data object at the start and pass that to plotly, then demo highlight on and change and show lasso, then add bscols to add table, then redo lasso select, then change height of table -->

```{r linked-brushing, exercise = TRUE}
mtcars %>% 
  highlight_key(~cyl) %>% 
  plot_ly(x = ~wt,
          y = ~mpg) %>% 
...
```{r brushed-linking-app-final, exercise = TRUE}
ggplot(data = mpg) + 
  geom_point(aes(x = displ,
                 y = hwy)) +
  labs(title = "Fuel economy from 1999 to 2008 for 38 car models",
...
<!-- NOTE: show different shapes, _ellipse (includes a non corvette, _circle, _rect) -->
...
<!-- NOTE: just adding highlight_key() -->

```{r graphical-query, exercise = TRUE}
gapminder %>% 
  group_by(country) %>% 
  plot_ly(x = ~year,
          y = ~lifeExp,
...
<!-- NOTE: copy previous ending code, demo shift (persistant = TRUE) first, then dynamic, then selectize and selectize + shift, then rename select bar -->

```{r graphical-query-more-features, exercise = TRUE}
```
...
```{r linked-brush-ex, exercise = TRUE}
```
...
<!-- NOTE: first create shared data, then replace datasets, name objects, add highlight(), then subplot, then subplot features (haven't grouped by anything, so gives dropdown of rownames (individual observations by default)), to name have to skip second argument, for this application don't want selectize and dynamic, so remove -->

```{r linking-multiple-plots, exercise = TRUE}
mtcars %>% 
...
share_data <- highlight_key(mtcars, , "Select a model")
...
<!-- NOTE: p = base plotly object, then add overall boxplot (x just gives the x axis a label), then by cut, then plot together; run through plots individually and then plot together, then add subplot() customizations -->

```{r subplot, exercise = TRUE}
p <- plot_ly(diamonds,
             y = ~price,
             color = I("black"), 
             alpha = 0.1 )
...
<!-- NOTE: add subplot, then customize, then make ggplotly, then make shared data -->

```{r subplot2, exercise = TRUE}
...
```{r highlight-vs-filter-preview, echo = FALSE}
```
...
<!-- NOTE: just create filter bar, then plot, then wrap in bscols, then add widths  -->

```{r highlight-vs-filter, exercise = TRUE}
...
```{r filtered-event-ex, exercise = TRUE}
