...
![](assignments/images/timeplot.png)
...
![](assignments/images/gas-prices-raw-data.png)
...
# load data
data_raw <- read.csv(file = "assignments/data/midwest-gas-prices.csv")
...
data_raw <- read_csv(file = "assignments/data/midwest-gas-prices.csv", col_names = TRUE, skip = 1)
...
The first column has the year/month combined followed by five pairs of columns for the dates and prices associated with weeks 1 through 5 of each month. Each of these five column pairs will need to be moved from wide format to long format using functions in the _tidyr_ package.
