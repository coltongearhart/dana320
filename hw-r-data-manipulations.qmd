...
We will be working with hypothetical student grade data [data-grades.csv](https://github.com/coltongearhart/dana320/blob/main/r/data/data-grades.csv), which contains information on two test scores from students of multiple sections for a single professor, and enrollment data [data-majors.csv](https://github.com/coltongearhart/dana320/blob/main/r/data/data-majors.csv), which has the students major.
...
# load data
data_raw <- read_csv(file = "r/data/data-grades.csv")
data_majors <- read_csv(file = "r/data/data-majors.csv")
...
2. Create an object named `data_summary_overall` that summarizes Test 1 scores with the sample size, average and standard deviation. Then add the following lines of code to add an indicator column and rearrange the columns.
...
2. Using the curved data, perform the necessary data steps and then recreate the following visual.
